...
@st.cache_data
def load_data():
    data = pd.read_csv('finaldata.csv')  # Update with the correct path to your CSV file
    return data
...
def find_best_context_and_response(data, user_input):
    ...
    tfidf_matrix = tfidf_vectorizer.fit_transform(data['Question'].tolist())
    ...
    best_context = data['Question'][best_index]
    best_response = data['Answer'][best_index]  # Get the corresponding response
...
def app():
    ...
    st.title("Chatbot with RAG for Cancer or healthcare")
